...
def serve(
    ...
    if metadata_server_cfg is not None or disagg_cluster_config is not None:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid server role: {server_role}. "
...
def disaggregated_mpi_worker(config_file: Optional[str], log_level: str):
    ...
    if (os.environ.get(
            DisaggLauncherEnvs.TLLM_DISAGG_RUN_REMOTE_MPI_SESSION_CLIENT)
            != "1"):
        set_cuda_device()
    ...
    if (os.environ.get(
            DisaggLauncherEnvs.TLLM_DISAGG_RUN_REMOTE_MPI_SESSION_CLIENT) == "1"
        ):
...
def _launch_disaggregated_leader(sub_comm, instance_idx: int, config_file: str,
    ...
    if (_child_p_global and hasattr(_child_p_global, "pid")
            and _child_p_global.pid is not None):
